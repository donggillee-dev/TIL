...
- render를 하기 이전에 내용을 Virtual DOM에 미리 적용 -> 그 다음에 그려준다

그래서 뭐?

이를 위해서는 우선 브라우저의 동작 원리를 알아야한다

### DOM Tree 생성

- HTML 전달받으면 렌더 엔진이 이를 파악해 DOM 노드로 이루어진 트리를 생성

### Render Tree 생성

- 외부 CSS 파일과 각각 스타일 정보를 이용해 DOM 트리에 따른 새로운 Render Tree 생성

### Layout

- 랜더 트리가 다 만들어지고 나면 거치는 과정
- 각 노드들의 스크린 좌표 주어지고 어디에 나타내야할지 위치가 주어짐

### Painting

- 렌더링 된 요소들에게 색을 입히는 작업
- 이걸 마치고 나면 스크린에 원하는 정보들이 나타나는 것



DOM을 조작할때마다 위 과정들이 계속 일어나는 것

요즘 트랜드인 SPA의 경우 DOM의 조작은 진짜 많이 일어난다

그래서 Virtual DOM을 사용하면 뭐가 장점인데??

### Virtual DOM

- 실제 DOM에 적용을 매번하면 연산이 많이 일어나니까
- Virtual DOM에서 필요한 연산들을 충분히 마친 후에
- 최종적인 결과를 실제 DOM에게 전달하는 것
- 브라우저 내에서 생기는 연산을 최대한 줄일 수 있는 것
